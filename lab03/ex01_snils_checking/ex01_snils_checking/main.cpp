...
bool validate(string snils) {
	if (snils.length() != 11 || !isDigits(snils)) { // snils must contain 11 chars and only digits
		return false;
	}
...
string getStringWithoutSpeials(string s) {
...
int main()
{
	...
	// remove whitespaces and defise
	snils = getStringWithoutSpeials(snils);

	return printResult(validate(snils));
}